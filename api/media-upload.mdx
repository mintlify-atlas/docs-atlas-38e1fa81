...
Baileys supports caching media to avoid re-uploading the same file:
...
// Store will cache media uploads
store.bind(sock.ev)

// First upload - uploads to WhatsApp servers
await sock.sendMessage(jid1, { image: buffer })

// Second upload of same media - may use cached URL
await sock.sendMessage(jid2, { image: buffer })
```
