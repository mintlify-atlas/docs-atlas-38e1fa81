...
<Warning>
  Baileys previously included `makeInMemoryStore`, but it's not recommended for production use as it stores everything in RAM. For production applications, implement a database-backed store.
</Warning>
